        ...
        <div class="mt-4 max-w-[1000px] mx-auto px-0">
            <div class="md:flex gap-4 justify-between mx-auto w-full">
                ...
                <div class="md:w-[60%] bg-white p-3 rounded-lg text-amber-950">
                    <div v-if="volumePromoData.volume_uid && volumePromoData.volume_num">
                        <p class="mb-2">{{ volumePromoData.graphic_novel_title }}</p>
                        <p class="mb-2 text-2xl">{{ volumePromoData.volume_name}}</p>
                        <p class="mb-2 text-xl">{{ volumePromoData.volume_title}}</p>
                        <p class="font-light  text-[22px] mb-2">
                            {{ volumePromoData.description }}</p>
                    </div>
                    ...
                    <br> 
                    <ul class="flex items-center pl-4 gap-4">
                      <li v-for="item in items" :key="item.id" @click="chooseLanguage(item)" 
                          class="hover:bg-gray-800 cursor-pointer px-4 py-2 flex items-center">
